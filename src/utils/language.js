...
export const flagFromCountryCode = (cc) => countryCodeMap[cc]?.flag || '';
export const languageFromCountryCode = (cc) => countryCodeMap[cc]?.language || cc;
export const iso639FromCountryCode = (cc) => countryCodeMap[cc]?.iso639;

export const findCountryCodes = (value) => {
  if (!value) return [];
  const result = [];
  for (const cc in countryCodeMap) {
    if (result.includes(cc)) continue;
    if (value.includes(countryCodeMap[cc].language)) result.push(cc);
  }
  return result;
};
