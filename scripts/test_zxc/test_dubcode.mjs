// Test dubCode parameter for English dub on Jujutsu Kaisen
...
async function getStreams(tmdbId, mediaType, season, episode, serverId, dubCode, dubType) {
  ...
  return { body: serversRes.body, statusCode: serversRes.statusCode };
}

console.log('=== Jujutsu Kaisen S01E01 - dubCode tests on 1icarus ===');
// Test default (no dub code)
console.log('\n--- No dubCode ---');
let r = await getStreams('95479', 'tv', 1, 1, '1icarus');
let d = JSON.parse(r.body);
console.log(`Status: ${r.statusCode}`);
console.log(`Links[0] link: ${d.links?.[0]?.link?.slice(0, 80)}`);
console.log(`Dubs (first 6):`, d.dubs?.slice(0, 6));

await sleep(3000);

console.log('\n--- dubCode=en, dubType=0 (English audio dub) ---');
r = await getStreams('95479', 'tv', 1, 1, '1icarus', 'en', 0);
d = JSON.parse(r.body);
console.log(`Status: ${r.statusCode}`);
console.log(`Links[0] link: ${d.links?.[0]?.link?.slice(0, 80)}`);
console.log(`Active dub:`, d.active);
console.log(`Dubs (first 6):`, d.dubs?.slice(0, 6));

await sleep(3000);

console.log('\n--- dubCode=ja, dubType=0 (Japanese audio = subbed) ---');
r = await getStreams('95479', 'tv', 1, 1, '1icarus', 'ja', 0);
d = JSON.parse(r.body);
console.log(`Status: ${r.statusCode}`);
console.log(`Links[0] link: ${d.links?.[0]?.link?.slice(0, 80)}`);
console.log(`Active dub:`, d.active);
...
console.log('\n=== Test dubCode=en, dubType=1 (English subtitle) ===');
r = await getStreams('95479', 'tv', 1, 1, '1icarus', 'en', 1);
d = JSON.parse(r.body);
console.log(`Status: ${r.statusCode}`);
console.log(`Links[0] link: ${d.links?.[0]?.link?.slice(0, 80)}`);
console.log(`Active dub:`, d.active);
...
console.log('\n=== Test 1resshin with dubCode=en, dubType=0 ===');
r = await getStreams('95479', 'tv', 1, 1, '1resshin', 'en', 0);
d = JSON.parse(r.body);
console.log(`Status: ${r.statusCode}`);
console.log(`Links: ${d.links?.length}, first link: ${d.links?.[0]?.link?.slice(0, 80)}`);
console.log(`Active dub:`, d.active);
