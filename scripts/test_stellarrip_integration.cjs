// Integration test for StellarRip source through the full project pipeline.
'use strict';
...
async function main() {
  ...
  const stellarrip = sources.find(s => s.id === 'stellarrip');
  if (!stellarrip) {
    console.error('FAIL: StellarRip source not registered');
    process.exit(1);
  }
  console.log('OK: StellarRip registered (label:', stellarrip.label + ')');
  ...
  for (const t of tests) {
    ...
    try {
      final = await Promise.race([
        streamResolver.resolve(ctx, [stellarrip], t.type, tmdbId),
        new Promise((_, rej) => setTimeout(() => rej(new Error('TIMEOUT 75s')), 75000)),
      ]);
    } catch (e) {
    ...
    for (const s of final.streams.slice(0, 5)) {
      ...
      console.log(`    url=${(s.url || s.externalUrl || '').slice(0, 90)}`);
      if (s.behaviorHints?.proxyHeaders?.request) {
        console.log(`    proxyHeaders: ${Object.keys(s.behaviorHints.proxyHeaders.request).join(',')}`);
      }
